        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden items-center space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link href="{{ route('noclegi.index') }}" :active="request()->routeIs('noclegi.*')">
                        Noclegi
                    </x-nav-link>
                    ...
                    <div x-data="{ dropdownOpen: false }" class="relative">
                        ...
                        <div x-show="dropdownOpen" 
                            ...
                            class="absolute left-0 mt-1 w-48 bg-white dark:bg-gray-800 rounded-md shadow-lg z-50 border border-gray-200 dark:border-gray-700 overflow-hidden">
                            <div class="py-1">
        ...
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if (Auth::user()->isAdmin())
                <x-responsive-nav-link href="{{ route('users.index') }}"
                    :active="request()->routeIs('users.index')">{{ __('translation.navigation.users') }}</x-responsive-nav-link>
            @endif

            @if (Auth::user()->isAdmin())
                <x-responsive-nav-link href="{{ route('attractions.index') }}"
                    :active="request()->routeIs('attractions.*')">{{ __('Atrakcje') }}</x-responsive-nav-link>
            @endif

            @if (Auth::user()->isAdmin())
                <x-responsive-nav-link href="{{ route('categories.index') }}"
                    :active="request()->routeIs('categories.*')">{{ __('Kategorie') }}</x-responsive-nav-link>
            @endif
        </div>
            ...
            <div class="mt-3 space-y-1">
                ...
                @if (Laravel\Jetstream\Jetstream::hasTeamFeatures())
                    ...
                    <x-responsive-nav-link href="{{ route('teams.show', Auth::user()->currentTeam->id) }}"
                        :active="request()->routeIs('teams.show')">{{ __('Team Settings') }}</x-responsive-nav-link>

                    @can('create', Laravel\Jetstream\Jetstream::newTeamModel())
                        <x-responsive-nav-link href="{{ route('teams.create') }}"
                            :active="request()->routeIs('teams.create')">{{ __('Create New Team') }}</x-responsive-nav-link>
                    @endcan
