<div class="p-6 space-y-4">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach ($noclegi as $n)
            <div class="bg-white shadow-lg rounded-xl overflow-hidden hover:shadow-2xl transition-all duration-300 group">
                
                {{-- Карусель фото з посиланням --}}
                    ...
                    <x-photo-carousel 
                        ...
                        containerClass="rounded-t-xl group-hover:scale-105 transition-transform duration-500"
                        arrowSize="w-8 h-8"
                ...
                <div class="p-6 space-y-4">
                    <h3 class="text-xl font-semibold text-gray-900">
                        <a href="{{ route('noclegi.show', $n->id) }}" class="hover:text-blue-600 transition">
                            {{ $n->title }}
                            <span class="inline-block opacity-0 group-hover:opacity-100 transform group-hover:translate-x-1 transition-all duration-200 ml-1">
                                →
                            </span>
                        </a>
                    </h3>
                    ...
                    <div class="space-y-3">
                        ...
                        @if (in_array($n->status, ['pending', 'rejected']))
                            <a href="{{ route('noclegi.edit', $n->id) }}"
                               class="inline-flex items-center justify-center gap-2 w-full px-4 py-3 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition font-medium shadow group/btn">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    {{-- Зручності --}}
                    <div class="flex flex-wrap gap-2">
                        @if ($n->has_kitchen) <span class="px-3 py-1 bg-gray-100 rounded-full text-sm group-hover:bg-gray-200 transition-colors duration-200">🍳 Kuchnia</span> @endif
                        @if ($n->has_parking) <span class="px-3 py-1 bg-gray-100 rounded-full text-sm group-hover:bg-gray-200 transition-colors duration-200">🅿️ Parking</span> @endif
                        @if ($n->has_bathroom) <span class="px-3 py-1 bg-gray-100 rounded-full text-sm group-hover:bg-gray-200 transition-colors duration-200">🚿 Łazienka</span> @endif
                        @if ($n->has_wifi) <span class="px-3 py-1 bg-gray-100 rounded-full text-sm group-hover:bg-gray-200 transition-colors duration-200">📶 Wi-Fi</span> @endif
                        @if ($n->has_tv) <span class="px-3 py-1 bg-gray-100 rounded-full text-sm group-hover:bg-gray-200 transition-colors duration-200">📺 TV</span> @endif
                        @if ($n->has_balcony) <span class="px-3 py-1 bg-gray-100 rounded-full text-sm group-hover:bg-gray-200 transition-colors duration-200">🌅 Balkon</span> @endif
                    </div>
                </div>
                ...
                <div class="p-4 border-t bg-gray-50 grid grid-cols-2 lg:grid-cols-4 gap-3">
                    <a href="{{ route('noclegi.show', $n->id) }}"
                       class="text-center py-2.5 rounded-lg bg-blue-100 text-blue-700 hover:bg-blue-200 transition font-medium group/btn">
                    ...
                    <a href="{{ route('noclegi.edit', $n->id) }}"
                       class="text-center py-2.5 rounded-lg bg-indigo-100 text-indigo-700 hover:bg-indigo-200 transition font-medium group/btn">
                        <svg class="w-5 h-5 mx-auto mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @if ($n->status === 'approved')
                        <a href="{{ route('noclegi.calendar', $n->id) }}"
                           class="text-center py-2.5 rounded-lg bg-green-100 text-green-700 hover:bg-green-200 transition font-medium group/btn">
                            <svg class="w-5 h-5 mx-auto mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button wire:click="deleteNocleg({{ $n->id }})" wire:confirm="Na pewno chcesz usunąć ten obiekt?"
                            class="text-center py-2.5 rounded-lg bg-red-100 text-red-700 hover:bg-red-200 transition font-medium group/btn">
                        <svg class="w-5 h-5 mx-auto mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
